        ...
        attrs?.also {
            ...
            try {
                ...
                if (a.hasValue(R.styleable.NavigationToolBarr_headerVerticalItemWidth)) {
                    verticalItemWidth = if (if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                                a.getType(R.styleable.NavigationToolBarr_headerVerticalItemWidth) == TypedValue.TYPE_DIMENSION
                            } else {
    ...
    private fun smoothOffset(offset: Int, duration: Long = collapsingBySelectDuration.toLong()) {
        ...
        offsetAnimator = ValueAnimator().also { animator ->
            ...
            animator.addUpdateListener {
                ...
                appBar?.also { if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                    it.postOnAnimation { it.requestLayout() }
                }
                }
            }
