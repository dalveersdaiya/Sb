...
public class SmoothCheckBox extends View implements Checkable
{
    ...
    private void init(AttributeSet attrs) {
        ...
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.SmoothCheckBox_stroke_width, DisplayHelper.convertDpToPx(getContext(), 0));
        ta.recycle();
    ...
    private int measureSize(int measureSpec) {
        int defSize = DisplayHelper.convertDpToPx(getContext(), DEF_DRAW_SIZE);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mTickPoints[2].x = Math.round((float) getMeasuredWidth() / 30 * 25);
        mTickPoints[2].y = Math.round((float) getMeasuredHeight() / 30 * 9);
